...
const ACCENT = "bg-amber-100 text-amber-800";
...
export default function KatanaOutput({ phase, logs, resultFile, backend }: KatanaOutputProps) {
  ...
  if (phase === "running") {
    return (
      <div className="space-y-4">
        {logs.length === 0 && (
          <p className="text-sm text-amber-700/80">กำลัง crawl… log จะแสดงเมื่อมีข้อมูล</p>
        )}
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2 max-h-[50vh] overflow-y-auto pr-1">
              {scored.map((entry, i) => (
                <div
                  key={i}
                  className="rounded-xl border border-zinc-200 bg-white p-3 shadow-sm hover:border-amber-200 transition"
                >
                  ...
                  <a
                    ...
                    className="mt-1.5 block font-mono text-sm text-amber-700 hover:text-amber-800 break-all underline underline-offset-2"
                  >
                    {entry.url}
                  </a>
                  {entry.reason && (
